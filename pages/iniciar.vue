...
const validData = computed(() => {
    return password.value.length > 5 && userEmail.value.length > 5;
});
...
const singInUser = () => {
    ...
    if (userExists) {
        if (userExists.password === password.value) {
            console.log(`Correct user, welcome back ${userExists.userEmail}`);
...
<template>
    <div>
        <h1 class="text-4xl text-center px-4 text-gray-900 mt-20 mb-10">Inicie su cuenta.</h1>
        <form @submit.prevent class="flex flex-col max-w-md m-auto gap-3">
            ...
            <div class="relative z-0 mb-1 w-full group px-4">
